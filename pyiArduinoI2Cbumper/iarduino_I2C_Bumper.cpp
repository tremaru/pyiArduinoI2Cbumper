...
bool	iarduino_I2C_Bumper::setCalibrationManual(uint16_t adc_1, uint16_t adc_2, uint16_t adc_3, uint16_t adc_4, uint16_t adc_5, uint16_t adc_6, uint16_t adc_7, uint16_t adc_8, uint16_t adc_9){	//	Параметры: граница АЦП между линией и фоном 1 датчика, 2 датчика, ... , 9 датчика.
	...
	//		Получение границы АЦП между линией и фоном для указанного датчика:														//	Возвращаемое значение:	граница АЦП от 0 до 4095.
uint16_t iarduino_I2C_Bumper::getCalibrationManual(uint8_t num){																//	Параметр:				номер датчика от 1 до 9.
			uint16_t result=0;																									//	Определяем переменную для хранения результата чтения.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Проверяем корректность параметра:																				//
				if(num==0||num>9){return 0;}																					//	Количество датчиков равно 9.
			//	Получаем данные:																								//
				if( _readBytes(REG_BUM_THRESHOLD + ((num-1)*2), 2) == false ){return 0;}										//	Читаем 2 байта из регистров «REG_BUM_THRESHOLD_X_L» и «REG_BUM_THRESHOLD_X_H» в массив «data».
...
float	iarduino_I2C_Bumper::getErrPID		(void){																				//	Параметр:				отсутствует.
			...
			//	Находим ошибку центрирования для линии:																			//
				for(uint8_t j=1; j<10; j++){ if(i&bit(j)){result+=f?0.5f:1.0f;f=true;}else{if(f){j=10;}else{result+=1.0f;}} }	//	Преобразуем биты «i» в значение «result» = -4.0, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4.0;
			//	Вычисляем ошибку если линия потеряна:																			//
				if( i==0 ){ result=flgErrPID?4.5f:-4.5f; }else{ flgErrPID=result>0; }											//	Если линия потеряна (i==0) то определяем значение «result» = ±4.5, а если линия не потеряна, то сохраняем флаг её положения относительно центра «flgErrPID».
			}	return result;																									//	Возвращаем результат.
}																																//

//		Получение ошибки центрирования края линии для ПИД регулятора:															//	Возвращаемое значение:	число от 0.0 (край линии по центру бампера) до ±4.5 (край линии потерян).
float	iarduino_I2C_Bumper::getSidePID		(int8_t dir){																		//	Параметр:				любое число, знак которого определяет край линии - левый, + правый.
			float result=-5;																									//	Определяем переменную для хранения результата.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Получаем данные:																								//
				uint16_t i = getLineDigital(BUM_LINE_ALL);																		//	Получаем флаги наличия линии под датчиками.
			//	Находим ошибку центрирования края линии:																		//
				for(uint8_t j=1; j<10; j++){ if(dir<0){result+=1.0f;if(i&bit(j)){j=10;}}else{if(i&bit(j)){result=-5.0f+j;}} }	//	Находим край линии от 0 до ±4.0.
			//	Вычисляем ошибку если линия потеряна:																			//
				flgErrPID=dir>0;																								//	Устанавливаем флаг flgErrPID по знаку dir, если скетч использует совместно функции getSidePID и getErrPID.
				if( i==0 ){ result=dir>0? 4.5f:-4.5f; }																			//	Если линия потеряна то определяем значение «result» = ±4.5, знак которого совпадает с «dir».
			}	return result;																									//	Возвращаем результат.
}																																//
																																//
//		Получение флага наличия перекрёстка или крутого поворота:																//	Возвращаемое значение:	флаг наличия перекрёстка под бампером (true/false).
bool	iarduino_I2C_Bumper::getCross		(uint8_t width, uint32_t time){														//	Параметры:				толщина линии трассы (количество датчиков над линией), [время в мс требуемое для преодоления перекрёстка].
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Получаем данные:																								//
				uint16_t i = getLineDigital(BUM_LINE_ALL);																		//	Получаем флаги наличия линии под датчиками.
				uint8_t k=0, n=0; for(uint8_t j=1; j<10; j++){ k+=i&bit(j)?1:0; n+=(i&bit(j-1)?1:0)==(i&bit(j)?1:0)?0:1;}		//	Определяем количество датчиков над линией «k» и количество переходов с фона на линию и обратно «n».
				if( k >= (width+2) || n > 2     ){ flg_Cross=1; tmr_Cross=millis(); }else										//	Устанавливаем флаг «flg_Cross» если линия под бампером «k» занимает на 2 датчика больше требуемой ширины линии «width» или количество переходов с фона на линию и обратно «n» больше двух.
				if( tmr_Cross < (millis()-time) ){ flg_Cross=0; }																//	Сбрасываем флаг «flg_Cross» если после съезда с перекрёстка прошло более «time» мс.
				return flg_Cross;																								//	Возвращаем флаг «flg_Cross».
			}	return false;																									//	Возвращаем ошибку.
}																																//
																																//
...
bool	iarduino_I2C_Bumper::_writeBytes	(uint8_t reg, uint8_t sum, uint8_t num){											//	Параметры:				reg - номер первого регистра, sum - количество записываемых байт, num - номер первого элемента массива data.
			...
			return result;																										//	Возвращаем результат записи (true/false).
																																...
																																//
void iarduino_I2C_Bumper::changeBus(std::string bus)
{
