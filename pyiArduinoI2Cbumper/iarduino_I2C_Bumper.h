...
#endif																																		//
																																			//
class iarduino_I2C_Bumper: public iarduino_I2C {																													//
	public:																																	//
	/**	Конструктор класса **/																												//
	...
	/**	Пользовательские функции **/																										//
		...
		uint16_t				getCalibrationManual	(uint8_t num					);													//	Объявляем  функцию получения границы АЦП между линией и фоном		(Параметр:  номер датчика для которого требуется узнать границу).
		...
		float					getSidePID				(int8_t dir						);													//	Объявляем  функцию возвращающую ошибку края  для ПИД регулятора		(Параметр:  любое число, знак которого определяет край линии - левый, + правый. Функцию удобно использовать для маневрирования на перекрёстках.
		bool					getCross				(uint8_t width, uint32_t time=0	);													//	Объявляем  функцию обнаружения перекрёстка или крутого поворота		(Параметры: толщина линии трассы (количество датчиков над линией), [время в мс требуемое для преодоления перекрёстка]).
